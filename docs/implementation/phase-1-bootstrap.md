...
- [x] Detect missing `~/Documents/Trayline/` on first launch and create the full skeleton:
  - `app-data/` (with `settings.json`, empty caches)
  - `skills/`
  - `skills/_system/`
  - `mcps/`
  - `projects/`
- [x] Bundle the two **system skills** with the Electron app and copy them into `skills/_system/` on first launch:
...
- [x] Restore system skills from bundled app resources on every launch if missing or corrupted
- [x] Build the **AI Terminal Adapter** layer:
...
- [x] Build the **runtime project metadata service** — reads/writes `project.json`, lists workflows, lists steps, finds skills, etc. All later phases query through this service, not the file system directly.
- [x] Minimal "Hello, Trayline is ready" splash that tells the user where its data lives (`~/Documents/Trayline/`)
