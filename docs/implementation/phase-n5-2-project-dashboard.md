...
- [x] **`project:live-stats` IPC handler** — scans all workflows for a given project and returns:
  ```ts
  {
...
- [x] **`project:check-readiness` IPC handler** — returns `{ ready: boolean, blockers: string[] }`:
...
- [x] **Project pill live stats display**: each pill in `ProjectListScreen` polls `project:live-stats` on mount and refreshes:
...
- [x] **Status dot state machine** — the dot in each pill reflects:
...
- [x] **Reactive update on `project:status-changed`**: renderer subscribes to this IPC push event in `ProjectListScreen` and updates the affected pill's `mounted` + `status` in local state without re-fetching the whole list

- [x] **"Pause all" / "Resume all" controls** — two small buttons at the top-right of the project list:
